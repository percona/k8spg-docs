...
!!! note
    ...
    Starting from 2.5.0, the Operator creates root CA on per-cluster basis. In Operator versions before 2.5.0, autogenerated certificates for all database clusters were based on the same generated root CA. 
...
3. Create a deployment. Replace the placeholders in the following command with your values:
    ...
    * Specify the Secret object with the TLS certificate from step 1
