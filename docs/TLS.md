...
You can also provide a custom root CA certificate to the Operator. In this case the Operator will not generate one itself, but will use the user-provided CA. 

!!! note

    Particularly, this can be useful if you would like to have several database clusters with certificates generated by the Operator based on the same root CA. Otherwise the Operator 2.5.0 and newer automatically creates root CA on per-cluster basis (in previous versions, autogenerated root CA was shared among all clusters under the Operator control).
