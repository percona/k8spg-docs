...
1. Create the backup on the version 1.x cluster, following the [official guide for manual (on-demand) backups :material-arrow-top-right:](https://docs.percona.com/percona-operator-for-postgresql/1.0/backups.html#making-on-demand-backup).
...
2. [Pause :material-arrow-top-right:](https://docs.percona.com/percona-operator-for-postgresql/1.0/pause.html) or delete the version 1.x cluster to ensure that you have the latest data.
    
    
    !!! warning 
    
        Before deleting the cluster, make sure that the [spec.keepBackups :material-arrow-top-right:](https://docs.percona.com/percona-operator-for-postgresql/1.0/operator.html#spec-keepbackups) Custom Resource option is set to `true`.
