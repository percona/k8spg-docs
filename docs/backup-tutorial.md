...
* In this tutorial we use the [AWS S3 :octicons-link-external-16:](https://aws.amazon.com/s3/) as the backup storage. You need the following S3-related information:
    ...
    * S3 credentials such as S3 key and secret to access the storage.
    ...
    If you don't have access to AWS, you can use any S3-compatible storage like [MinIO :octicons-link-external-16:](https://min.io/docs/minio/linux/index.html). Check the list of [supported storages](backups.md#backup-storage). Find the [storage configuration guides](backups-storage.md#storage-setup-guides) for each backend.
