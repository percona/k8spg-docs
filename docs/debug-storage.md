...
* If you are going to rely on the Operator [storage scaling functionality](scaling-vertical.md#storage-resizing-with-volume-expansion-capability), ensure the storage class supports PVC expansion (it should have  `allowVolumeExpansion: true` in the output of the above command).
