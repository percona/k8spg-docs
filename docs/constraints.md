...
* `backups.volumeSnapshots.jobs.tolerations` — snapshot restore prepare jobs. The Operator applies these tolerations to the Job that prepares volumes before restoring from a PVC snapshot.
