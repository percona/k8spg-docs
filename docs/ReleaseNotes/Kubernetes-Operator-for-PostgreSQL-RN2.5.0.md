...
* {{ k8spgjira(598) }}: Database major version upgrade now [supports custom images](../update-db-major.md)
* {{ k8spgjira(560) }}: A `pg-restore` Custom Resource is now automatically created at [bootstrapping a new cluster from an existing backup](../backups-clone.md#make-a-full-data-clone)
* {{ k8spgjira(555) }}: The Operator now creates separate Secret with CA certificate for each cluster
* {{ k8spgjira(553) }}: Users can provide the Operator with their own [root CA certificate](../tls-manual.md#generate-custom-certificates-for-the-operator-yourself)
...
With the Operator versions prior to 2.5.0, [autogenerated TLS certificates](../TLS.md#automatic-certificate-generation-by-the-operator) for all database clusters were based on the same generated root CA. Starting from 2.5.0, the Operator creates root CA on a per-cluster basis.
