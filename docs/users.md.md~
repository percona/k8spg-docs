...
You can set role privileges by using the standard [role attributes :material-arrow-top-right:](https://www.postgresql.org/docs/current/role-attributes.html) that PostgreSQL provides and adding them to the `spec.users.options` subsection in the Custom Resource. 
